...
- Windows (PowerShell): `.\run.ps1 <command>`
...
- Restart the stack to pick up code changes: `./run.sh restart` or `.\run.ps1 restart`
- Relaunch the UI: `./run.sh ui` or `.\run.ps1 ui`
- If you change dependencies or the Dockerfile, run `build` again before `restart`.

## Command reference

All commands below work with either `./run.sh` or `.\run.ps1`:
...
```
Adjust the model as needed. This file is ignored by git.
